...
class PerfTestDriver:
    ...
    def __init__(self, tinfo, use_parallel_search, language="c", timing_code=''):
        ...
        print (self.tinfo.build_cmd)
        # parse build command to get -DXXX 
        split_cmd = self.tinfo.build_cmd.split()
        define_option = ' '.join([c for c in split_cmd if c.startswith('-D')])
        self.extra_compiler_opts += define_option 
        # hardcode postbuild command for data generation
        # TODO postbuild command should parse from annotion
        self.tinfo.post_build_cmd = 'python /home/users/kewen/bsAnalysis/depAnalysis/repo/CFGPass/script/preprocess.py' 
    ...
    def __build(self, perf_params=None, coord=None):
        ...
        if self.tinfo.post_build_cmd:
            ...
            if timer_objfile or self.tinfo.libs:
                # include libs and objectfile
                cmd = ('%s -extra %s %s -- %s %s' % (self.tinfo.post_build_cmd, timer_objfile, self.tinfo.libs, self.src_name, self.extra_compiler_opts))
            else:
                cmd = ('%s -- %s %s' % (self.tinfo.post_build_cmd, self.src_name, self.extra_compiler_opts))
            # TODO: log all commands
            print (cmd)
            status = os.system(cmd)
            if status:
