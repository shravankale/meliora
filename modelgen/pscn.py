...
import utils
from custom_errors import BadShapeError,BadAttriDimError,BadDummyValueDef,ModelNotFoundError
import os
...
class PSCN():
    def __init__(self,w,s=1,k=10
        ...
        has_model : already has a trained model. Only needs to load 
        """
        self.w=w
        self.s=s
        self.k=k
        ...
        if not has_model:
			...
			# Wrapper model is not able to save/load directly
			# This is a workround,
			# only save/load Keras model and then re-create wrapper
            self.model = KerasClassifier(build_fn=self.load,epochs=self.epochs, 
                                     batch_size=self.batch_size, verbose=self.verbose)
			# Model is loaded after fit function,
			# so maunally set
            self.model.model = self.load(model_path)
        ...
        if np.array([self.dummy_value]).ravel().shape[0]!=self.original_dim: 
    ...
    def predict(self,X):
        ...
        preds = self.model.predict_proba(X_preprocessed)
        end_time = time.time()
        print ('Time for prediction in s ', end_time-start_time)
	...
	# only save and load Keras model with build-in function
    def save(self, path):
        self.model.model.save(path)

    def load(self, path):
        if not os.path.exists(path):
            raise ModelNotFoundError('The model does not exist') 	
        else:
