...
if __name__ == '__main__':
    ...
    parser.add_argument('-lp', '--lpath', help='absolute path of the custom cfg library',
						...
						default='/home/users/kewen/bsAnalysis/depAnalysis/repo/CFGPass/build/cfgpass/libCfgPass.so')
    ...
    print(cmd_bc)
	# generate data
    cmd_gen = ['opt-9', '-load', lib_path, '-cfggen', '-disable-output', file_name+'.bc']
    ...
    Command(cmd_bc).run('', shell=False)
    print ('start generating data')
    Command(cmd_gen).run('', cwd=instru_dir, shell=False)
    t_end = time.time()
