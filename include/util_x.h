...
llvm::StringRef getCIDV(Loop *l){
    //Get cannonical induction variable of Loop *l
    return l->getCanonicalInductionVariable()->getName();
}
llvm::StringRef getIDV(Loop *l, ScalarEvolution &se){
    //
    return l->getInductionVariable(se)->getName();
}
...
//         2 - target to process
int checkSkip(Loop* l, ScalarEvolution& se) {

    //std::cout<<"Inside checkSkip()"<<endl;

    int ret = 0;
   
    //x_ changing from Meliora
    //string name = getIdv(l);
    string name = getIDV(l,se).str();
    std::cout<<"Loop Induction variable: "<<name<<endl;
    if (!name.empty()) {
        // check two pre set variables
        if (name.substr(0,6) == "orio_i" || name == "_t") {
            ret = 1;
        } 
        else if (name.substr(0, 5) == "loop_") {
            // check fake loop induction variable 
...
void getTargetLoops(LoopInfo& li, vector<Loop*>& loopVec, ScalarEvolution& se) {
...
void getTargetLoops(LoopInfo* li, vector<Loop*>& loopVec) {
    ...
    for (LoopInfo::iterator it = li->begin(); it != li->end(); it++) {
        ...
        while (!stk.empty()) {
            ...
            int isTarget = checkSkip_kewen(lp);
